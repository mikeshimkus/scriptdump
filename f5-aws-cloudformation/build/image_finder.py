...
parser.add_argument('-u', '--profile', help='Optionally provide cli profile to use when querring images in US aws')
parser.add_argument('-c', '--china_profile', help='Optionally provide cli profile to use when querring images in china aws')
parser.add_argument('-g', '--gov_profile', help='Optionally provide cli login profile to use when querring images in us gov aws')
args = parser.parse_args()

# Constants
# Versions to update, update whenever AMIs or BIGIP/IQ versions are updated
BIGIP_MARKETPLACE_VERSION = args.bigip_version or "15.0.1-0.0.11"
BIGIQ_MARKETPLACE_VERSION = args.bigiq_version or "7.0.0.1.0"
...
BYOL_BOOT_LOCATION_MAP = {
    "8f2ed1fb-93bb-4f06-a8f5-eb84757d5fab": "LTMOneBootLocation",
    "d3260d79-3eae-461c-a45f-3c2de70ea351": "LTMOneBootLocation",
    "6d69f6a1-37da-4a16-846c-aeb7cc0402aa": "LTMTwoBootLocations",
    "4fd89ae5-f32f-4499-95ea-2c9a255a3e83": "LTMTwoBootLocations",
    "0d93696e-a18e-466c-b9c3-6534c1d5fa7b": "AllOneBootLocation",
    "ca00deda-341d-4b7d-aebb-540ea28702a8": "AllOneBootLocation",
    "65df-4235-b79c-a3ea049dc1db": "AllTwoBootLocations",
    "529f-4eca-95bf-d8b3f5813e61": "AllTwoBootLocations"
}
...
class BigIpImageFinder(object):
    def __init__(self):
        ...
        self.packages = ['(good)', '(better)', '(best)', '(payg - waf)', '(payg-adv waf)', '(per app ve - ltm)', '(pve ltm)', '(per app ve - advanced waf)', '(pve adv waf)', '(payg-hp adv waf)', '(payg-hp ltm)']
        self.throughputs = ['([0-9]+gbps)', '([0-9]+mbps)']
        self.licenses = ['(byol)', '(payg)']
        self.bigip_version_regex = [
            # 13.1.1-0.0.4
            'bigip-([1-9][0-9].[0-9].[0-9]-[0-9].[0-9].[0-9][1-9])',
            'bigip-([1-9][0-9].[0-9].[0-9].[0-9]-[0-9].[0-9].[0-9][1-9])'
        ]
        self.bigiq_version_regex = [
            # 6.0.0.0.0.x
            'big-iq-([1-9].[0-9].[0-9].[0-9].[0-9])'
        ]
        ...
        for region in kwargs['regions']:
            ...
            for image in images:
                ...
                # AMIs for PAYG BIGIP
                    ...
                    if (image['package'] in PACKAGE_CPU_MAP and
                        image['cpu'] in PACKAGE_CPU_MAP['payg-hp ltm'] and
                            PACKAGE_CPU_MAP[image['package']][image['cpu']] not in package_dict):
