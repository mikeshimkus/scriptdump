...
def build_cloudlibs_sh(iApp_verify, comment_out):
    cloudlibs_sh = [
        "\n",
        "#!/bin/bash",
        ...
        str(comment_out) + "declare -a filesToVerify=(\"/config/cloud/f5-cloud-libs.tar.gz\" \"/config/cloud/f5-cloud-libs-aws.tar.gz\" \"/config/cloud/f5-appsvcs-3.5.1-5.noarch.rpm\" \"/config/cloud/aws/f5.service_discovery.tmpl\" \"/config/cloud/aws/f5.cloud_logger.v1.0.0.tmpl\"" + str(iApp_verify) + ")",
...
def build_init_commands(ha_type, loglevel, components, license_type, BIGIP_VERSION, template_name, version):
...
def create_launch_config_metadata(ha_type, cloudlib_url, cloudlib_aws_url, as3_url, cloud_logger_url, discovery_url, lines, cloudlibs_sh, waitthenrun_sh, get_nameserver, loglevel, components, license_type, BIGIP_VERSION, template_name, version):
    mode = '000755'
    owner = 'root'
    group = 'root'
    # files={
    ...
    files = {
        ...
        '/config/cloud/f5-appsvcs-3.5.1-5.noarch.rpm': {'source': as3_url},
    ...
    d = build_init_commands(ha_type, loglevel, components, license_type, BIGIP_VERSION, template_name, version)
...
def add_resources_autoscale(ha_type, t, restrictedSrcAddress, managementGuiPort, restrictedSrcAddressApp, Vpc, cloudlib_url, cloudlib_aws_url, as3_url, cloud_logger_url, discovery_url, lines, comment_out, loglevel, components, license_type, BIGIP_VERSION, template_name, version):
    ud = Base64(Join('', [
                "#!/bin/bash -x\n",
    ...
    cloudlibs_sh = build_cloudlibs_sh('', comment_out)
...
def main():
    ...
    version = '5.2.1'
    # Big-IP mapped
    BIGIP_VERSION = '15.0.1-0.0.11'
    # Cloudlib Branch
    branch_cloud = 'v4.12.0'
    branch_aws = 'v2.5.0'
    branch_cloud_iapps_sd = 'v2.3.2'
    branch_cloud_iapps_logger = 'v1.0.0'
    # AS3 branch and package
    branch_as3 = 'v3.6.0'
    package_as3 = 'f5-appsvcs-3.5.1-5.noarch.rpm'
...
# Cloudlib and iApp URL
    ...
    as3_url = "http://cdn.f5.com/product/cloudsolutions/f5-appsvcs-extension/" + str(branch_as3) + "/dist/lts/" + str(package_as3)
    ...
    # BEGIN RESOURCES
    if ha_type == "autoscale":
        add_resources_autoscale(ha_type, t, restrictedSrcAddress, managementGuiPort, restrictedSrcAddressApp, Vpc, cloudlib_url, cloudlib_aws_url, as3_url, cloud_logger_url, discovery_url, lines, comment_out, loglevel, components, license_type, BIGIP_VERSION, template_name, version)
    else:
        if network:
        ...
        if bigip:
            ...
            discovery_policy = [{"Effect": "Allow", "Action": ["ec2:DescribeInstances", "ec2:DescribeInstanceStatus", "ec2:DescribeAddresses", "ec2:AssociateAddress", "ec2:DisassociateAddress", "ec2:DescribeNetworkInterfaces", "ec2:DescribeNetworkInterfaceAttribute", "ec2:DescribeRouteTables", "ec2:ReplaceRoute", "ec2:assignprivateipaddresses", "sts:AssumeRole", ], "Resource": ["*"]}]
            if license_type == "bigiq":
        ...
        "variables": [ \
            ...
            { \
                ...
                if ha_type == "across-az":
                    iApp_verify = " \"/config/cloud/aws/f5.aws_advanced_ha.v1.4.0rc5.tmpl\""
                ...
                cloudlibs_sh = [
                    ...
                    str(comment_out) + "fi",
                    str(comment_out) + "echo loaded verifyHash",
                    str(comment_out) + "declare -a filesToVerify=(\"/config/cloud/f5-cloud-libs.tar.gz\" \"/config/cloud/f5-cloud-libs-aws.tar.gz\" \"/config/cloud/" + str(package_as3) + "\" \"/config/cloud/aws/f5.service_discovery.tmpl\" \"/config/cloud/aws/f5.cloud_logger.v1.0.0.tmpl\"" + str(iApp_verify) + ")",
                    str(comment_out) + "for fileToVerify in \"${filesToVerify[@]}\"",
                    str(comment_out) + "do",
                    str(comment_out) + "    echo verifying \"$fileToVerify\"",
                    str(comment_out) + "    if ! tmsh run cli script verifyHash \"$fileToVerify\"; then",
                        ...
                        for number in range(3, 8):
                            network_config += [
                                If("createNic" + str(number),
                                    "GATEWAY_MAC" + str(number) + "=`ifconfig eth" + str(number) + " | egrep ether | awk '{print tolower($2)}'`;GATEWAY_CIDR_BLOCK" + str(number) + "=`curl -s -f --retry 20 http://169.254.169.254/latest/meta-data/network/interfaces/macs/${GATEWAY_MAC" + str(number) + "}/subnet-ipv4-cidr-block`;GATEWAY_PREFIX" + str(number) + "=${GATEWAY_CIDR_BLOCK" + str(number) + "#*/};",
                                    ""
                                   )
                            ]
                    network_config += [
                ...
                if ha_type != "standalone" and (BIGIP_INDEX + 1) == CLUSTER_SEED:
                    ...
                    if ha_type == "across-az":
                        custom_sh += [
                            "tmsh load sys application template /config/cloud/aws/f5.aws_advanced_ha.v1.4.0rc5.tmpl\n",
                            "if [[ $VIPEIP ]]; then\n",
                            "   tmsh create /sys application service HA_Across_AZs template f5.aws_advanced_ha.v1.4.0rc5 tables add { eip_mappings__mappings { column-names { eip az1_vip az2_vip } rows { { row { ${VIPEIP} /Common/${EXTPRIVIP} /Common/${PEER_EXTPRIVIP} } } } } } variables add { eip_mappings__inbound { value yes } }\n",
                ...
                init_dict = {
                    ...
                    '/config/cloud/f5-appsvcs-3.5.1-5.noarch.rpm': InitFile(
                        source=as3_url,
                        mode='000755',
                        owner='root',
                        group='root'
